...
#X_train is a tensor of size (num_train_examples, num_samples, num_channels)
X_train = np.load(inputFile + '_x.npy')
#Since we're doing auto-encoding, Y is equal to X
...
#here create CNN + LSTM (CNN per block size of 10sec*44.1Khz, LSTM across CNN's, final output CNN from last output and state of LSTM)
hidden_dims = config['hidden_dimension_size']

graph = network_utils.create_cnn_network(input_shape, input_length)
...
while cur_iter < num_iters:
    ...
    #history = graph.fit({'input':X_train, 'output1':y_train}, nb_epoch=10)
